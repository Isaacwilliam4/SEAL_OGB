...
def get_pos_neg_edges(split, split_edge, edge_index, num_nodes, percent=100):
    ...
    if "pos_edge_label" in split_edge["train"]:
        pos_edge = split_edge[split]["pos_edge_label"].t()
        ...
        if split == "train":
            target_neg = torch.randint(
                0, num_nodes, [target.size(0), 1], dtype=torch.long
            )
        else:
        ...
        neg_edge = torch.stack(
            [source.repeat_interleave(neg_per_target), target_neg.view(-1)]
        )
